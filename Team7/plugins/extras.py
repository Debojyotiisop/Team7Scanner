...
from Team7.plugins.officials import add_enforcers, add_inspector, rem_enforcers, rem_inspector
from Team7 import ENFORCERS, INSPECTORS, session
...
try:
    from Vortex import HEROKU_API_KEY, HEROKU_APP_NAME
...
async def addenf(event) -> None:
    ...
    if u_id in ENFORCERS:
        await System.send_message(event.chat_id, "That person is already Armature!")
        return
    if HEROKU:
    ...
    await System.send_message(
        event.chat_id, f"Added [{u_id}](tg://user?id={u_id}) to Armature"
    )
...
async def rmenf(event) -> None:
    ...
    if HEROKU:
        ...
        await System.send_message(
            event.chat_id, "Removed from Armature, Restarting..."
        )
        await restart()
    await System.send_message(
        event.chat_id, f"Removed [{u_id}](tg://user?id={u_id}) from Armature"
    )


@System.on(system_cmd(pattern=r"armature", allow_inspectors=True))
...
async def join(event) -> None:
    ...
    if private:
        ...
        await System.send_message(
            Sibyl_logs,
            f"{(await event.get_sender()).first_name} made Vortex System join {private.group(5)}",
        )
    else:
...
async def addins(event) -> None:
    ...
    if u_id in INSPECTORS:
        await System.send_message(event.chat_id, "That person is already an Veteran!")
        return
    if HEROKU:
        config["INSPECTORS"] = os.environ.get("INSPECTORS") + " " + str(u_id)
    else:
        await add_inspector(u_id)
        await System.send_message(event.chat_id, "Added to Veteran, Restarting...")
        await restart()
    await System.send_message(
        event.chat_id, f"Added [{u_id}](tg://user?id={u_id}) to VETERAN"
    )
...
async def rmins(event) -> None:
    ...
    if u_id not in INSPECTORS:
        await System.send_message(event.chat_id, "Is that person even an Veteran?")
        return
    u_id = str(u_id)
    if HEROKU:
        ...
        await System.send_message(
            event.chat_id, "Removed from Veteran, Restarting..."
        )
        await restart()
    await System.send_message(
        event.chat_id,
        f"Removed Veteran status of [{u_id}](tg://user?id={u_id}), Now that user is a Armature.",
    )
...
async def leave(event) -> None:
    ...
    if c_id:
        ...
        await System.send_message(
            event.chat_id, f"Vortex left id[-{c_id.group(1)}]"
        )
    else:
        await System(LeaveChannelRequest(link))
        await System.send_message(event.chat_id, f"Vortex has left chat[{link}]")
