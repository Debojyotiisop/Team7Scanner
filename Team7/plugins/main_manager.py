...
async def force_(event):
    try:
        ...
        return await event.reply("Add a reason gay.")
    if event.is_reply:
...
async def scan(event, flags):
    ...
    if flags.a:
        ...
        await event.reply("Oooof! New sban call, call hes been sent, wait for Veterans or Devs to accept.")
