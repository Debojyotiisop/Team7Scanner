...
def get_data_from_url(url: str) -> bool | tuple[str, str]:
    """
...
async def scan(event, flags):  # sourcery no-metrics
...
async def approve(event, flags):  # sourcery no-metrics
    replied = await event.get_reply_message()
...
Commands:
    `scan` - Reply to a message WITH reason to send a request to Inspectors/Team7 for judgement
    `approve` - Approve a scan request (Only works in TEAM7 System Base)
