...
for load in to_load:
    try:
        imported = importlib.import_module("Vortex.plugins." + load)
...
async def status(event):
    msg = await event.reply("Connecting to Vortex Database")
    time.sleep(1)
